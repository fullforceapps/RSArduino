...
*****************************************************************************************/
byte mac[] = { 0xDE, 0x05, 0x97, 0x2C, 0x61, 0xB3 }; // Unique MAC
// byte mac[] = { 0x00, 0x90, 0xA9, 0xA1, 0x9E, 0xD6 }; // WDTVLiveHub MAC
            ...
            if (client.available()) {   // client data available to read
                ...
                if (c == '\n' && currentLineIsBlank) {
                    ...
                    if(StrContains(filename, ".htm") || !StrContains(filename, ".")) {
                        ...
                        client.println();
                        webFile.open(&root, filename, O_READ);
                                            //Serial.println(F("HTM File Opened"));
                    } else if (StrContains(filename, ".gif") || StrContains(filename, ".bmp") || StrContains(filename, ".png") || StrContains(filename, ".jpg") || StrContains(filename, ".ico")) {
                                            //Serial.println(F("GIF File Exists"));
                        webFile.open(&root, filename, O_READ);
                                            //Serial.println(F("Opened GIF File"));
                        if (webFile.isOpen()) {
                                            //Serial.println(F("Verify Open GIF File"));
                            client.println(F("HTTP/1.1 200 OK"));
                            client.println();
                        }
                    }
...
//                    if (webFile.isOpen()) {
//                        Serial.println(F("webfile open!"));
//                        while ((c = webFile.read()) >= 0) {
//                            client.print((char)c);
//                        }
//                        webFile.close();
//                    }
